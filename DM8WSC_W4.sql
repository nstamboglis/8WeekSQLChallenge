...
-- DA CAPIRE COME CALCOLARE CUMULAZIONE DELL'INTERESSE DALLA TABELLA SOTTO (HINT: valori negativi da trattare)
...
-- NOTE: The solution below assumes that customers won't pay negative interest AND the interest is simple, not compound
...
-- the solution below allows for compound interest
-- NOTE: The solution below assumes that customers won't pay negative interest AND the interest is simple, not compound
with balance_table as (
	select 
		balance_raw.calendar_date,
...
select 
	tab2.calendar_date,
	tab2.customer_id,
	tab2.balance_clean + tab2.balance_int_adj as balance_int,
	case 
		when (tab2.balance_clean + tab2.balance_int_adj) * 6/365 * row_number() over(partition by customer_id) > 0 then (tab2.balance_clean + tab2.balance_int_adj) * 6/365 * row_number() over(partition by customer_id)
		else 0
	end balance_int_compund
from(
	select 
		tab1.calendar_date,
		tab1.customer_id,
		tab1.balance_clean,
		case 
			when tab1.balance_int > 0  then tab1.balance_int
			when tab1.balance_int <= 0 then 0
		end as balance_int_adj
	from(
			select 
